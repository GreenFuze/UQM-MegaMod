...
AiPlayerResponseInput (ENCOUNTER_STATE *pES)
{
	...
	{	/* Show why before falling back. A silent switch back to the menu
		 * looks like the game hanging, which is what it looked like when the
		 * Claude login had quietly expired. */
		const char *why = AiConv_LastError ();
