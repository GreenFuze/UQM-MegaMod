...
/* Resolves a relative sidecar directory against the EXECUTABLE's location
 * rather than the current directory.
 *
 * The game is routinely launched from somewhere else - a shell in another
 * folder, a shortcut, a debugger - and a cwd-relative path silently fails
 * there, leaving the player on the original dialogue menu with no clue why. */
static const char *
resolveWorkingDir (const char *workingDir, char *buf, size_t cap)
{
	...
	if (workingDir[1] == ':' || workingDir[0] == '\\' || workingDir[0] == '/')
		return workingDir;

	len = GetModuleFileNameA (NULL, exePath, sizeof (exePath));
	...
	*slash = '\0';
	...
	snprintf (buf, cap, "%s\\%s", exePath, workingDir);
	return buf;
}
...
AiProc_Spawn (const char *workingDir, char *errBuf, size_t errCap)
{
			...
			CREATE_NO_WINDOW, NULL, dir, &si, &pi))
	{
		snprintf (errBuf, errCap,
				"CreateProcess failed (%lu) in '%s'; is Python on PATH?",
				(unsigned long)GetLastError (), dir != NULL ? dir : "(null)");
		CloseHandle (stdoutRd);
		CloseHandle (stdoutWr);
		CloseHandle (stdinRd);
		CloseHandle (stdinWr);
