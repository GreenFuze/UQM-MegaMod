"""Everything a conversation provider does that is not vendor-specific.

The prompts, the reply contract, the schema check and its two retries, and
the rule that a listed line is spoken AS THE CAPTAIN all belong to the game
rather than to whichever model happens to be answering. They live here so
that a second backend is a _complete method and nothing else.

A subclass supplies two things: how to turn a system prompt and a user prompt
into text, and how to describe a failure to a player.
"""
...
class ConversationProvider(LLMProvider):
    """A provider answering with the game's own JSON reply contract.

    Subclasses implement _complete. Everything else is shared, so the
    behaviour a player sees does not drift between backends.
    """

    @abstractmethod
    def _complete(self, system_prompt: str, user_prompt: str) -> str:
        """One completion, synchronously. May raise; callers wrap it."""

    @staticmethod
    def _describe(exc: Exception, last_result: str = "") -> str:
        """Turn a backend failure into something a player can act on."""
        detail = (last_result or "").strip()
        return str(exc) if not detail else f"{exc}: {detail}"
    ...
    def generate(self, request: ConverseRequest, system_prompt: str) -> ConverseResponse:
        ...
        if promises and response.action is None:
            ...
            try:
                raw = self._complete(system_prompt, retry)
    ...
    def summarise(self, system_prompt: str, transcript: str) -> str:
        ...
        """
        try:
            raw = self._complete(system_prompt, transcript)
