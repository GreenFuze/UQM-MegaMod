...
| **An Anthropic API key** | Conversation is billed to your own account — see [Cost and licensing](#cost-and-licensing) |
...
Set your key once per session (or set it permanently in your environment):
...
Conversation is billed to **your own Anthropic API account**. No key is bundled; you
create one at [console.anthropic.com](https://console.anthropic.com/settings/keys) and
set `ANTHROPIC_API_KEY`. Expect roughly 2–10k input tokens per turn depending on how
much of the story the character has unlocked.

**A Claude Pro / Max subscription cannot be used for this.** Anthropic's Agent SDK
terms are explicit:
